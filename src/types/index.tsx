...
export interface Item {
   ...
   * of falling back to whichever store has the cheapest known price.
   * null = no override configured - falls back to cheapest price.
   * Pre-filled from the item's category default store (see
   * CategoryDefaultStore below) at CREATION time only; editable
   * independently afterward on the product form, and once explicitly set,
   * always takes priority over the category default - the category
   * default never retroactively changes an existing item.
   */
  defaultStoreId: string | null;
}

/**
 * One category's configured default store, managed from the Price
 * Catalog's "Default store by category" card. Only affects NEW products
 * created in that category afterward (pre-fills their Item.defaultStoreId)
...
export interface ConsolidatedItem {
   ...
   * items split across multiple recipes/sources, toggling flips every
   * contributing row together (see CartItem's handleToggleItemChecked).
   */
  isCheckedCheckout: boolean;
  /**
   * Which underlying row the item-level pantry-override control applies
   * to, used for Home Mode's single "already have" stepper regardless of
   * source count. Prefers the manual "Others" row (sourceRecipeId null) if
...
export interface RecipeIngredient {
  ...
  defaultPrice: number;
  /** True if defaultStoreId came from an explicit custom route, false if it's the Default (auto) fallback (item default store, or cheapest price if the item has none). */
